...
function filterLongRoutes(routes: RouteSummary[]): RouteSummary[] {
  ...
  const times = routes
    .map((r: RouteSummary) => r.totalMinutes ?? Infinity)
  ...
  const filtered = routes.filter((r: RouteSummary) => {
...
function sortByTime(routes: RouteSummary[]): RouteSummary[] {
  ...
  // 첫 번째만 "최적" 표시
  return sorted.map((r: RouteSummary, idx: number) => ({
    ...r,
    isBest: idx === 0,
  }));
}
...
export default function HomePage() {
  ...
  const displayRoutes = useMemo<RouteSummary[]>(() => {
    ...
    return normalizedRoutes.map((r: RouteSummary, idx: number) => ({
      ...r,
      isBest: idx === 0,
    }));
  }, [normalizedRoutes]);
    ...
    if (displayRoutes.length > 0) {
      console.log(
        "[DISPLAY ROUTES - path[0] 기반]",
        {
          ...
          routes: displayRoutes.map((r: RouteSummary) => ({
